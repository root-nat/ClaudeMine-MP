...
/**
 * Hand-rolled loot for a buried-treasure chest: the iconic Heart of the Sea plus a pile of treasure. Deterministic for
 * a given Random.
 */
final class BuriedTreasureLoot{
	...
	public static function roll(Random $random) : array{
		$items = [VanillaItems::HEART_OF_THE_SEA()]; //always present, like vanilla
		$rolls = 3 + $random->nextBoundedInt(4); //+3..6 treasure stacks
		for($i = 0; $i < $rolls; ++$i){
	...
	private static function pick(Random $random) : Item{
		...
		if($roll < 80){
			return VanillaItems::PRISMARINE_CRYSTALS()->setCount(1 + $random->nextBoundedInt(3));
		}
		if($roll < 90){
