...
/**
 * Records a raid's valid victims - survival players, villagers and iron golems - within range as {@link TargetCandidate}s
 * into NEAREST_ENTITIES, so a raider hunts the whole village rather than only the nearest player. Used by {@link
 * \pocketmine\entity\Raider}. The only world access is World::getNearbyEntities.
 */
final class RaiderTargetSensor implements Sensor{
	...
	public function sense(Living $owner, Memory $memory) : void{
		...
		foreach($owner->getWorld()->getNearbyEntities($bb, $owner) as $entity){
			...
			$isVictim = ($entity instanceof Player && $entity->isSurvival())
