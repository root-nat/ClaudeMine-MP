...
/**
 * A wolf's combat goal: chases and bites whatever was chosen in COMBAT_TARGET by {@link \pocketmine\entity\ai\sensor\DefendOwnerSensor}.
 * Identical in mechanics to {@link MeleeAttackGoal} but reads the wolf-only COMBAT_TARGET slot rather than the shared
 * ATTACK_TARGET, so the pack never turns on nearby players the generic target selector would otherwise pick. Registered at
 * the wolf's highest priority so combat preempts panicking, following its owner and idle wandering.
 */
final class WolfAttackGoal extends BaseGoal{
	...
	private function target(MobContext $mob) : ?TargetCandidate{
		if($mob instanceof Wolf && $mob->isSitting()){
			//stop the instant the wolf is told to sit, without waiting for the sensor's next scan to clear the slot
			return null;
		}
		$target = $mob->getMemory()->get(MemoryModuleType::COMBAT_TARGET);
	...
	public function start(MobContext $mob) : void{
		...
		//don't carry a half-spent strike cooldown over from a previous, now-finished engagement
